...
function drawFirstPageHeader(doc, title, updatedAt, glpiId) {
  ...
  return hY + rh * 4 + 5; // Y after header
}
...
function drawRunningHeader(doc, title) {
  ...
  try { doc.addImage('data:image/jpeg;base64,' + LOGO_B64, 'JPEG', ML + 2, hY + 1.5, 22, 6); } catch {}
...
function drawFooter(doc, pageNum, totalPages) {
  ...
  const full = 'Restricted use: IT department only. External disclosure prohibited.';
  const ruEnd = doc.getTextWidth('Restricted use');
  const startX = PW / 2 - doc.getTextWidth(full) / 2;
  doc.text(full, PW / 2, fy, { align: 'center' });
